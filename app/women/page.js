...
function Women() {
    ...
    useEffect(() => {
        ...
        async function fetchWomenData() {
            try {
                setLoading(true)
                const response = await axios.get(`${API_URL}/api/women`)
                if (response.data.success === true) {
                    setWomens(response.data.womenData)
                } else {
                    ...
                    <aside className="w-full lg:w-1/5 px-4 py-6 rounded-lg bg-gray-50">
                        ...
                        <ul className="space-y-2">
                            ...
                            <li>
                                <label className="flex items-center">
                                    <Checkbox
                                        checked={selectCategory === "Penty"}
                                        onCheckedChange={(checked) => {
                                            setSelectCategory(checked ? 'Penty' : '');
                                        }}
                                    />
                                    <span className="ml-2">Penty</span>
                                </label>
                            </li>
                            <li>
                                <label className="flex items-center">
