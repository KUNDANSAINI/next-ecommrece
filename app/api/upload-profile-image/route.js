...
export async function POST(req) {
    try {
        ...
        const pathOfImage = path.join(process.cwd(), './public/profile', filename);
        await writeFile(pathOfImage, bufferData)
        
        const imageName = `/profile/${filename}`
        
        if (!imageName) {
            return NextResponse.json({ success: false, message: "Image Not Uploading!" })
        }

        return NextResponse.json({ success: true, message: "Profile Images Successfully Uploaded!", imageName })
    } catch (error) {
